...
{
    /// <summary>
    /// Serve a popolare la prima volta il DB (DB SEEDING)
    /// </summary>
    public class StoreSeedData : DropCreateDatabaseIfModelChanges<StoreEntities>
    {
        protected override void Seed(StoreEntities context)
        {
            ...
            GetCredentials().ForEach(z => context.Login.Add(z));
        ...
        private static List<Login> GetCredentials()
        {
            return new List<Login>
            {
                new Login {Username= "blinkettaro", Password= "Admin.01"}
            };
        }
